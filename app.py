...
def calcular_metricas(y_true, y_pred):
    ...
    return {
        ...
        "Especificidade": cm[0, 0] / cm[0].sum() if cm[0].sum() != 0 else 0
    }

# Simulação de retorno em porcentagem
def simular_retorno(y_true, y_pred, lucros):
    ...
    total_lucros_possiveis = lucros[y_true == 1].sum()
    if total_lucros_possiveis == 0:
        return 0, 0, 0
    pct_ganhos = retorno_ganhos / total_lucros_possiveis * 100
    pct_perdas = retorno_perdas / total_lucros_possiveis * 100
    pct_total = retorno_total / total_lucros_possiveis * 100
    return pct_ganhos, pct_perdas, pct_total
...
if grafico == "Série Temporal Completa":
    ...
    fig = px.bar(
        ...
        color=["Baixa", "Alta"],
        color_discrete_map={"Baixa": "red", "Alta": "green"}
    )
    st.plotly_chart(fig, use_container_width=True)
    st.write(f"**Alta:** {dist.get(1,0):.2f}% | **Baixa:** {dist.get(0,0):.2f}%")
